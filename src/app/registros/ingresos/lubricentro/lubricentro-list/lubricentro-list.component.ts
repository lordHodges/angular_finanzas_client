...
import { ExcelService, IngresoLubricentroService } from '@app/_services';
...
export class LubricentroListComponent implements OnInit {
  ...
  columnDefs = [
    ...
    { field: 'correo', sortable: true, filter: true },
    { field: 'tipoVehiculo', sortable: true, filter: true },
    { field: 'ppu', sortable: true, filter: true },
    { field: 'marca', sortable: true, filter: true },
    { field: 'modelo', sortable: true, filter: true },
    { field: 'anio', sortable: true, filter: true },
    ...
    { field: 'nAutorizacion', sortable: true, filter: true },
    ...
    if (this.selectedRows.length <= 1 && this.selectedRows.length > 0) {
      this.selectedRows.forEach((x) => {
        ...
        this.ingresoService
          ...
          .pipe()
          .subscribe((x) => {
            this.rowData2 = x[0].RespaldoIngresoLubricentros;
            console.log('data2', x);
          });
      ...
      alert(
        `Atención: No es posible visualizar ${this.selectedRows.length} Registros`
      );
    }
  }
}
